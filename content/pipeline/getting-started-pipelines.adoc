...
*Multibranch Pipeline* projects expose the name of the branch being built with the BRANCH_NAME environment variable. They also provide a special *checkout scm* Pipeline command, which checks out the specific commit that the Jenkinsfile originated, so that branch integrity is automatically mainta*ined.

== Using credentials 

To read about using credentials, follow the link below

https://github.com/jenkins-infra/jenkins.io/edit/master/content/doc/book/using/using-credentials.adoc
